...
# realized trial sample size, number of participants assigned to the treatment arm, number of participants 
...
RAR_ComputeStat = function(w, potential.outcomes, u.bound, l.bound, symmetric.bounds = TRUE, response.probs, cohort.size){
  ...
  if(symmetric.bounds == TRUE){
    ...
    if(sum(w[,1] > u.bound)>0) {efficacy=1; harm=0} #treatment efficacious
    if(sum(w[,1] < l.bound)>0) {efficacy=0; harm=1} #treatment harmful
  }
...
Run.Trial.Get.Stats = function(response.probs, cohort.size, nmax, j, PP.mod, u.bound, l.bound, symmetric.bounds = TRUE){
...
Ex.RAR.nmax = find.nmax(nmax=153,ntrials=100,null.RR=c(0.12,0.12),alt.RR=c(0.37,0.12),j=10,bound.type = "Pocock",PP.mod="restrict")

### simulate 1000 trials and get stats - ALT
Ex.RAR.alt = data.frame(do.call(rbind, lapply(1:1000, function(x){
  set.seed(x)
  Run.Trial.Get.Stats(response.probs = c(0.37,0.12), cohort.size = 200, nmax = Ex.RAR.nmax[[1]],
                      j=10, PP.mod="optimal", u.bound = Ex.RAR.nmax[[4]], l.bound = 1-Ex.RAR.nmax[[4]])})))

### simulate 1000 trials and get stats - NULL
Ex.RAR.null = data.frame(do.call(rbind, lapply(1:1000, function(x){
  set.seed(x)
