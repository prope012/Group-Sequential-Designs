...
# A trial is stopped early if the posterior probability that the treatment is better than the 
# control exceeds a stopping boundary. The following metrics are computed:
...
simulate.freq.trial.PO <- function(potential.outcomes, nmax, k, rand.prob=0.5, max.deviation = 3){
...
prop_test_get_stats_fn = function(trial, interim.test=c("fisher","chisquare","uncondExact"), potential.outcomes,
                                  l.bound, u.bound, symmetric.bounds = TRUE, ns, k, cohort.size, response.probs){
...
Run.Trial.Get.Stats.Freq = function(response.probs, bound.probs, cohort.size, k, bound.type, symmetric.bounds = TRUE,
  ...
  if(symmetric.bounds == FALSE & bound.type == "asymmetric-OF"){
    ...
    t1e = function(c,k){
      t=1:k/k; b=c*sqrt(1/t); a = c*(2-sqrt(1/t)) 
      sum(gsProbability(k=k,n.I=t,a=a,b=b,theta=0)$upper$prob)
    } 
    
    ### function to get multiplicative constant a that controls one-sided type I error at 0.025
    get.constant = function(alpha=0.025) uniroot(function(x) alpha-t1e(c=x,k=k),interval=c(0,4))$root
    a <- get.constant()
    u.bound <- sqrt(k/1:k)*a
    l.bound <- a*(2-sqrt(k/1:k))
    
    ### function to compute type II error rate for same design that controls type I error rate at 0.025
    t2e = function(theta,alpha=0.025,k){
      c=get.constant(alpha); t=1:k/k; b=c*sqrt(1/t); a = c*(2-sqrt(1/t)) 
      sum(gsProbability(k=k,n.I=t,a=a,b=b,theta=theta)$lower$prob)
    } 
    
    ### function to get sample size inflation factor (R = n.gs/n.fix)
    get.inf.factor = function(beta=0.10,alpha=0.025,k) (uniroot(function(x) beta-t2e(theta=x,alpha=alpha,k=k),interval=c(0,10))$root/(qnorm(1-alpha)+qnorm(1-beta)))^2
    R = get.inf.factor(k=k)
    fixed.ss = 2*(power.prop.test(n = NULL, p1 = bound.probs[1], p2 = bound.probs[2], sig.level = 0.05, power = 0.90, alternative = "two.sided"))$n
    gs.ss <- fixed.ss*R
    ns <- round(seq(gs.ss/k, gs.ss, gs.ss/k))
  }
  ...
  if(symmetric.bounds == FALSE & bound.type == "asymmetric-PK"){
    ...
    ### function to compute type I error rate for gs design
    t1e = function(c,k){
      t=1:k/k; b=rep(c,k); a = c*(2-sqrt(1/t)) 
      sum(gsProbability(k=k,n.I=t,a=a,b=b,theta=0)$upper$prob)
    } 
    ...
    ### function to compute type II error rate for same design that controls type I error rate at 0.025
    t2e = function(theta,alpha=0.025,k){
      c=get.constant(alpha); t=1:k/k; b=rep(c,k); a = c*(2-sqrt(1/t)) 
      sum(gsProbability(k=k,n.I=t,a=a,b=b,theta=theta)$lower$prob)
    } 
  ...
  stats = prop_test_get_stats_fn(trial=trial, interim.test = interim.test, potential.outcomes = potential.outcomes,
                                 l.bound = l.bound, u.bound = u.bound, symmetric.bounds = symmetric.bounds, ns=ns, k=k, 
...
Ex.Freq.alt = data.frame(do.call(rbind, lapply(1:100, function(x){
  set.seed(x)
  Run.Trial.Get.Stats.Freq(response.probs = c(0.37,0.12), bound.probs = c(0.37,0.12), cohort.size=447, k=10, bound.type = "OF",
...
Ex.Freq.null = data.frame(do.call(rbind, lapply(1:100, function(x){
  set.seed(x)
  Run.Trial.Get.Stats.Freq(response.probs = c(0.12,0.12), bound.probs = c(0.37,0.12), cohort.size=447, k=10, bound.type = "OF",
                           symmetric.bounds = TRUE, interim.test = "uncondExact")})))