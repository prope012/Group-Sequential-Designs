...
# a specific number of interim analyses J. If symmetric boundaries are undesirable, the user may implement 
...
#     2) OBF-like boundaries: we will use c_j = pnorm(sqrt(J/j)*a), where c_j is the value of the jth stopping 
...
#        u_j = pnorm(a) and lower boundary equal to l_j = pnorm(2a-sqrt(J/j)*a) where a is an appropriate constant that 
...
#        u_j = pnorm(sqrt(J/j)*a) and lower boundary equal to l_j = pnorm(2a-sqrt(J/j)*a) where a is an appropriate constant that 
#        leads to a one-sided type I error rate of 0.025.
...
###   - ns = the sample size at each interim analysis (generally equal to round(seq(nmax/j,nmax,by=nmax/j))) 
...
###   - a j x 5 matrix containing the posterior probability that the treatment is better than the control,
...
###   - t1e = type I error rate of the design
###   - pow = power of the design
...
###   - The power of the design arising from the identified stopping boundaries is then found

find.pow = function(nmax,ntrials,null.RR,alt.RR,j,bound.type,PP.mod){
  ...
  if(bound.type=="asymmetric-OBF" | bound.type=="asymmetric-PK" ){
    ...
    foo = rep(0,length(a))
    for(i in 1:length(a)){
      if(bound.type=="asymmetric-OBF"){u.bound[[i]] <- pnorm(sqrt(j/1:j)*a[i])} else{u.bound[[i]] <- rep(pnorm(a[i]),j)}
      l.bound[[i]] <- pnorm(2*a[i]-sqrt(j/1:j)*a[i])
...
###   - if the power of the design is less than 0.89 or greater than 0.91, the "find.pow" function is re-performed
###     using a new estimate of nmax
###   - The above step is repeated until 0.89<power<0.91

find.nmax = function(nmax,ntrials,null.RR,alt.RR,j,bound.type,PP.mod){
  Pow.res = find.pow(nmax=nmax,ntrials=ntrials,null.RR=null.RR,alt.RR=alt.RR,j=j,bound.type = bound.type, PP.mod = PP.mod)
...
### Determine nmax for a Bayesian RAR gs trial with a target effect size of 25% and J=10,5,3
...
set.seed(1994)
RAR.Pocock.10.TE25 = find.nmax(nmax=180,ntrials=100,null.RR=c(0.12,0.12),alt.RR=c(0.37,0.12),j=10,bound.type = "Pocock",PP.mod = "restrict")
RAR.Pocock.5.TE25 = find.nmax(nmax=146,ntrials=100,null.RR=c(0.12,0.12),alt.RR=c(0.37,0.12),j=5,bound.type = "Pocock",PP.mod = "restrict")
RAR.Pocock.3.TE25 = find.nmax(nmax=139,ntrials=100,null.RR=c(0.12,0.12),alt.RR=c(0.37,0.12),j=3,bound.type = "Pocock",PP.mod = "restrict")