...
###   - bound.type = "OF" or "Pocock"; specifies whether to use an OBF-like or Pocock-like stopping boundary
###   - PP.mod = specify which RAR adaptive modification to use ("restrict","exponent","neyman","optimal");
###              "exponent" refers to the modification that uses the tuning parameter
...
###   - pow = power of design
###   - stop.bounds = stopping boundaries calibrating type I error rate at 0.05
###   - null.sim = trial results for the null scenario simulations
###   - alt.sim = trial results for the alternative scenario simulations

### Purpose of function:
###   - This function finds the stopping boundaries that calibrate type I error at 0.05 
...
### Determine nmax for a Bayesian RAR trial with a target effect size of 25% and K=10,5,3
### We recommend starting with nmax equal to the maximum sample size from the gsDesign package
### for the analogous classical frequentist design

### below is an example for finding an appropriate initial value for nmax for scenario 1
library(gsDesign)
